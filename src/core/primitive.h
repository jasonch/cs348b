...
class HeightFieldPrimitive : public Primitive {
public:
	HeightFieldPrimitive(const Point& pt1, const Point& pt2, 
	...
	/** Definition not necessary in the following methods **/
    const AreaLight *GetAreaLight() const 
		{ return NULL; }
    BSDF *GetBSDF(const DifferentialGeometry &dg,
        const Transform &ObjectToWorld, MemoryArena &arena) const 
    ...
    BSSRDF *GetBSSRDF(const DifferentialGeometry &dg,
        const Transform &ObjectToWorld, MemoryArena &arena) const
		{ return NULL; }
	const Point *getPts() const { return pts; }
private:
	// each primitive represents a plane indicated by its 4 corners
	// these are the tiles that make up a terrain
	Point pts[4];
};
