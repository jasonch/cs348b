...
void DirectLightingIntegrator::Preprocess(const Scene *scene, const Camera *camera,
	...
	const RealisticCamera* rcam = NULL;
	PointLight* plight = NULL;
	try {
		rcam = dynamic_cast<const RealisticCamera*>(camera);
		for (size_t i = 0; i < scene->lights.size(); i++) {
			plight = dynamic_cast<PointLight*>(scene->lights[i]);
			if (plight != NULL) 
				break;
		}

	} catch (std::exception &e) {
		printf("Exception in creating lens flare: %s \n", e.what());
		return;
	}

	if (rcam == NULL || plight == NULL) return;

	rcam->GenerateLensFlare(scene, plight);
