...
SubsurfaceMaterial *CreateSubsurfaceMaterial(const Transform &xform,
	...
	// new parameters for matte BRDF 
	Reference<Texture<Spectrum> > Kd = mp.GetSpectrumTexture("Kd", Spectrum(0.1f));
	Reference<Texture<float> > sigma = mp.GetFloatTexture("sigma", 0.f);
	return new SubsurfaceMaterial(scale, Kr, sigma_a, sigma_prime_s, ior, bumpMap, Kd, sigma);
}
